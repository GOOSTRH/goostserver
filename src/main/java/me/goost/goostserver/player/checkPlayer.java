...
public class checkPlayer {
    public static void checkPlayer(Player player){
        ...
        }else if(Job.Job.get(player.getUniqueId()).equals("beaksu") ){
            // hobo
            setHealthStats(player,1.0,1.0,1.0);
            setDefStats(player,0);
            setManaStats(player,0,0);

        }else if(Job.Job.get(player.getUniqueId()).equals("archer") ){
            // archer
            setHealthStats(player,100.0,100.0,20.0);
            setDefStats(player,100);
            setManaStats(player,200,200);
        ...
        }else if(Job.Job.get(player.getUniqueId()).equals("night") ){
            // night
